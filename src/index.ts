...
import SolarDataLayerOverlay from "./api/overlays";
import { findClosestBuildingInsights, getDataLayers } from "./api/solar-api";
...
export default class SolarPanelsMap {
  ...
  constructor(private readonly apiKey: string, private readonly element: HTMLDivElement) {
    element.classList.add("woolpert-solar");
  ...
  private async initGoogleMap() {
    ...
    this.mapElement.className = "woolpert-solar-map";
  ...
  private async initAutoComplete() {
    ...
    this.formElement.className = "woolpert-solar-form active";
    ...
    inputElement.className = "woolpert-solar-form-input";
    ...
    autocomplete.addListener("place_changed", () => {
      ...
      this.showInsightsForCoordinate(place.geometry.location).then(() => {
  ...
  private initInfoPanel() {
    ...
    dataContainer.className = "woolpert-solar-panel woolpert-solar-panel-data";
    this.panelElement.append(dataContainer);

    const panelsCountContainer = document.createElement("div");
    panelsCountContainer.className = "woolpert-solar-panel-container";
    dataContainer.append(panelsCountContainer);

    const panelsCountTitle = document.createElement("h3");
    panelsCountTitle.className = "woolpert-solar-panel-title";
    ...
    this.panelCountConicElement.className = "woolpert-solar-panel-conic";
    ...
    this.panelCountConicInnerElement.className = "woolpert-solar-panel-conic-inner";
    ...
    annualEnergyContainer.className = "woolpert-solar-panel-container";
    dataContainer.append(annualEnergyContainer);

    const annualEnergyTitle = document.createElement("h3");
    annualEnergyTitle.className = "woolpert-solar-panel-title";
    ...
    this.panelEnergyConicElement.className = "woolpert-solar-panel-conic";
    ...
    this.panelEnergyConicInnerElement.className = "woolpert-solar-panel-conic-inner";
    ...
    sliderContainer.className = "woolpert-solar-panel";
    this.panelElement.append(sliderContainer);

    this.panelSliderElement = document.createElement("input");
    this.panelSliderElement.className = "woolpert-solar-panel-slider";
  ...
  public async showInsightsForCoordinate(coordinate: google.maps.LatLng) {
    this.formElement.innerHTML = `<h2 class="woolpert-solar-form-label">Loading data, can take up to a minute !...</h2>`;
    ...
    this.map.moveCamera({
      center: this.buildingInsights.center,
      zoom: 21
    });

    const radius = Math.max(google.maps.geometry.spherical.computeDistanceBetween(
      this.buildingInsights.boundingBox.ne, 
      this.buildingInsights.boundingBox.sw) / 2, 100);

    const dataLayerOverlay = await SolarDataLayerOverlay.getDataLayersCanvas(this.apiKey, coordinate, radius);
    ...
    solarPanelConfig.roofSegmentSummaries.forEach((roofSegmentSummary) => {
      this.buildingInsights.solarPotential.solarPanels.filter((solarPanel) => solarPanel.segmentIndex === roofSegmentSummary.segmentIndex).slice(0, Math.min(solarPanelConfig.panelsCount - panelsCount, roofSegmentSummary.panelsCount)).forEach((solarPanel) => {
        ...
        if (!this.solarPanelPolygonReferences.has(solarPanel)) {
          ...
          const top = google.maps.geometry.spherical.computeOffset(solarPanel.center, height, angle + 0);
          const right = google.maps.geometry.spherical.computeOffset(solarPanel.center, width, angle + 90);
          const left = google.maps.geometry.spherical.computeOffset(solarPanel.center, width, angle + 270);
